(function() {
    ...
    //Проверка вводимых данных на 
    document.querySelector('#phone').addEventListener('keydown', checkValidNumbers);

    function checkValidNumbers(e) {
        if (e.keyCode >= 48 && e.keyCode <= 57) {} else if (e.keyCode == 107) {} else if (e.keyCode == 8) {} else {
            e.preventDefault();
        }
        if (e.target.value.length >= 13 && e.keyCode != 8) {
            e.preventDefault();
        }
